...
export function negotiationDialog(ui) {
  ...
  <section class="p-negotiation" role="dialog" aria-modal="true" aria-labelledby="p-negotiation-title" tabindex="-1">
    ...
    ${evidence ? demoEvidence(ui.pricingAnalysis) : guided && !draft && !sourcing ? demoFramework() : draft ? `<div class="p-draft-address"><span>To</span><strong>${sourcing ? 'Alternative supplier · choose after qualification review' : `${esc(product.contact)} · ${esc(product.supplier)}`}</strong><label for="p-message-subject">Subject</label><input id="p-message-subject" value="${esc(subject)}"></div><label class="p-draft-label" for="p-message-body">Message</label><textarea id="p-message-body" spellcheck="true">${esc(draftText)}</textarea><p class="p-panelnote">Editable, copy-only negotiation draft. Nothing is sent from this dialog.</p>` : sourcing ? rfqContent(product, flow) : `
    ...
    ${guided ? `<footer class="p-negotiation__footer"><p id="p-draft-feedback" role="status">${draft ? 'Nothing sent · LAA stays internal' : evidence ? 'Illustrative evidence · buyer validation needed' : 'Next: prepare the supplier conversation'}</p><div>${draft ? '<button type="button" class="p-button" data-demo-agents="true">Back to Agents</button><button type="button" class="p-button p-button--brand" data-copy-draft="true">Copy draft</button>' : evidence ? '<button type="button" class="p-button" data-demo-overview="true">Back to Overview</button><button type="button" class="p-button p-button--brand" data-demo-plan="true">Build negotiation framework →</button>' : '<button type="button" class="p-button" data-demo-evidence="true">Back to evidence</button><button type="button" class="p-button p-button--brand" data-demo-handoff="true">Continue to Agents →</button>'}</div></footer>` : `<footer class="p-negotiation__footer"><p id="p-draft-feedback" role="status">${draft ? 'You review and lead the conversation' : sourcing ? 'Preparation only · no suppliers contacted' : 'Your limits stay internal'}</p><div>${draft ? '<button type="button" class="p-button" data-plan-back="true">Back to framework</button><button type="button" class="p-button p-button--brand" data-copy-draft="true">Copy draft</button>' : sourcing ? '<button type="button" class="p-button" data-plan-back="true">Back to framework</button>' : '<button type="button" class="p-button" data-find-alternatives="true">Find alternatives</button><button type="button" class="p-button p-button--brand" data-draft-message="true">Draft supplier message →</button>'}</div></footer>`}
  </section>`;
}
